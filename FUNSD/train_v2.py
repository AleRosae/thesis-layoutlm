...
def preprocess_data(examples):
  images = [Image.open(path).convert("BGR") for path in examples['image_path']]
  ...
  for epoch in range(num_train_epochs):  
    ...
    if current_loss > last_loss:
      ...
      if trigger_times >= patience:
          ...
          df.to_csv(f'results/v2/log_v2_FUNSD_run{run}.csv', index = False)
          return best_model
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default = 5e-5, help='Learning rate for training. Default is 3e-5')
    parser.add_argument('--epochs', type=int, default=100, help='Number of epochs. Default is 25')
    parser.add_argument('--patience', type=int, default=100, help='Patience. Default is 5')
    ...
    torch.save(model, 'models/LayoutLMv2_FUNSD.pt')
